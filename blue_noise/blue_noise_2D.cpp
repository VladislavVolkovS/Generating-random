...
float Comp(std::vector<std::vector<Color>>& mask, int ix, int iy) {
    float sum[size] = {};
#pragma omp parallel for schedule(dynamic, 1) num_threads(7)
    for (int oy = -7; oy <= 7; ++oy) {
        for (int ox = -7; ox <= 7; ++ox) {
            ...
            const float da = std::abs(mask[ix][iy][0] - mask[sx][sy][0]);
            const float db = std::abs(mask[ix][iy][1] - mask[sx][sy][1]);
            const float b = std::sqrt(da * da + db * db) / (sigma_s * sigma_s);
...
int main()
{
    ...
    for (int iy = 0; iy < size; ++iy) {
        for (int ix = 0; ix < size; ++ix) {
            ...
            mask[ix][iy] = Color{r, g, 0};
        }
    }

    SaveCSV(mask, "visual/start_noise_2D.csv");
    ...
    for (int ite = 0; ite < kMaxIteration; ++ite) {
        ...
        const float e0 = Comp(mask, px, py);
        const float e1 = Comp(mask, qx, qy);
    ...
    SaveCSV(mask, "visual/blue_noise_2D.csv");
